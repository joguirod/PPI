...
function removerHashtag() {
    let hashtags = document.getElementById("hashtags");
    let selecionada = hashtags.selectedOptions;

    if (selecionada === null || selecionada.length === 0) {
        exibirErro("erroHashtag", "Selecione uma hashtag para remover");
        return;
    }

    hashtags.removeChild(selecionada[0]);
}
